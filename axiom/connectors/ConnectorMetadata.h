...
class Column {
 ...
 protected:
  ...
  // containing Schema is not in use. Mutable for the same reason as latestStats_.
  mutable std::vector<std::unique_ptr<ColumnStatistics>> allStats_;

 private:
  // Serializes changes to statistics. Mutable for the same reason as latestStats_.
