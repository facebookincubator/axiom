...
CreateTableOptions parseCreateTableOptions(
  ...
  if (it != options.end()) {
    ...
    VELOX_USER_CHECK_EQ(
        delimiter.size(), 1, "field.delim must be a single character");
...
std::shared_ptr<LocalTable> createLocalTable(
  ...
  if (createTableOptions.serdeOptions.has_value()) {
    ...
    // Store null string for TEXT format
    if (!serdeOpts.nullString.empty()) {
      serdeParameters
...
TablePtr LocalHiveConnectorMetadata::createTable(
  ...
  auto table =
      createLocalTable(tableName, rowType, createTableOptions, hiveConnector());
...
void LocalHiveConnectorMetadata::loadTableFromPath(std::string_view tableName) {
  std::lock_guard<std::mutex> l(mutex_);
