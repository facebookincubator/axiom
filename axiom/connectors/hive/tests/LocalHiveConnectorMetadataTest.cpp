...
TEST_F(LocalHiveConnectorMetadataTest, complexTypeStats) {
  ...
  LOG(INFO) << "structVector->size() = " << structVector->size()
            << ", totalStructs = " << totalStructs;

  // Create array of structs with varying sizes 1-4
  std::vector<vector_size_t> structOffsets = {0};
  for (int i = 0; i < kTestSize - 1; ++i) {
    structOffsets.push_back(structOffsets.back() + ((i % 4) + 1));
  }
  // Add the final offset to mark the end
  structOffsets.push_back(totalStructs);
  LOG(INFO) << "structOffsets.size() = " << structOffsets.size()
            << ", last = " << structOffsets.back();
  auto arrayStructVector = makeArrayVector(structOffsets, structVector);
  LOG(INFO) << "arrayStructVector->size() = " << arrayStructVector->size();
  ...
  LOG(INFO) << "mapData.size() = " << mapData.size();

  // Convert to proper map vector structure
  std::vector<vector_size_t> mapOffsets = {0};
  ...
  LOG(INFO) << "mapOffsets.size() = " << mapOffsets.size()
            << ", mapVector->size() = " << mapVector->size();
  ...
  auto& mapStats = stats[1];
  EXPECT_GT(mapStats.avgLength.value_or(0), 0);
  ASSERT_EQ(2, mapStats.children.size()); // map has 2 children (keys, values)

  // First child: keys (integers)
  auto& keysStats = mapStats.children[0];
  EXPECT_GT(keysStats.numValues, 0);
