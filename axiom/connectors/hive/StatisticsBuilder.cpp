...
class ArrayStatisticsBuilder : public StatisticsBuilder {
 public:
  explicit ArrayStatisticsBuilder(
    ...
    elementBuilder_ =
        StatisticsBuilder::create(arrayType->elementType(), options);
  }
...
class MapStatisticsBuilder : public StatisticsBuilder {
 public:
  explicit MapStatisticsBuilder(
    ...
    valueBuilder_ = StatisticsBuilder::create(mapType->valueType(), options);
  }
...
void ArrayStatisticsBuilder::add(const velox::VectorPtr& data) {
  ...
  auto* arrayVector = data->asUnchecked<velox::ArrayVector>();
  for (auto i = 0; i < arrayVector->size(); ++i) {
    ++numRows_;
    if (arrayVector->isNullAt(i)) {
      ++nullCount_;
    } else {
      auto arraySize = arrayVector->sizeAt(i);
...
void StructStatisticsBuilder::add(const velox::VectorPtr& data) {
  ...
  auto* rowVector = data->asUnchecked<velox::RowVector>();
...
void MapStatisticsBuilder::add(const velox::VectorPtr& data) {
  ...
  auto* mapVector = data->asUnchecked<velox::MapVector>();
  for (auto i = 0; i < mapVector->size(); ++i) {
    ++numRows_;
    if (mapVector->isNullAt(i)) {
      ++nullCount_;
    } else {
      auto mapSize = mapVector->sizeAt(i);
