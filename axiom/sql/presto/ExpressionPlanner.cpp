...
ExpressionPlanner::WindowScope::WindowScope(
  ...
  const auto savedOutputAliases = planner_.outputAliases_;
  auto restoreOutputAliases =
      folly::makeGuard([&] { planner_.outputAliases_ = savedOutputAliases; });
  planner_.clearOutputAliases();
  ...
  for (const auto& definition : definitions) {
    ...
    AXIOM_PRESTO_SEMANTIC_CHECK(
        ...
        definition->name()->value());
    auto resolved = planner_.resolveWindow(definition->window());
    ...
    planner_.windowDefinitions_.emplace(
        name, NamedWindow{std::move(resolved), std::move(spec)});
  }
...
lp::WindowSpec ExpressionPlanner::convertWindow(
  ...
  (void)resolveWindow(window);
  const auto name = canonicalizeIdentifier(*window->existingWindowName());
  const auto& definition = windowDefinitions_.at(name);
  if (window->isWindowReference()) {
    return definition.spec;
  }

  return convertWindowParts(window, options, definition.spec);
}
...
std::shared_ptr<Window> ExpressionPlanner::resolveWindow(
  ...
  const auto& referenced = it->second.resolved;
  if (window->isWindowReference()) {
    return referenced;
  }
  ...
  AXIOM_PRESTO_SEMANTIC_CHECK(
      ...
      identifier->value());
  AXIOM_PRESTO_SEMANTIC_CHECK(
      referenced->orderBy() == nullptr || window->orderBy() == nullptr,
      window->location(),
      identifier->value(),
  ...
  return std::make_shared<Window>(
      window->location(),
      referenced->partitionBy(),
      window->orderBy() != nullptr ? window->orderBy() : referenced->orderBy(),
      window->frame());
}
