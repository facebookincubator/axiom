...
void reducingJoinsRecursive(
  ...
  for (auto join : joinedBy(candidate)) {
    if (join->leftOptional() || join->rightOptional() || join->unnest()) {
      continue; // These joins typically don't reduce result cardinality.
    }
...
float startingScore(PlanObjectCP table) {
  ...
  if (table->is(PlanType::kUnnestTableNode)) {
    ...
    // Because it's right side of unnest join edge and they are non-commutative.
  }
