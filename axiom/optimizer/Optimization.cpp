...
void Optimization::applySingleDistinctTransformation(
    RelationOpPtr& plan,
    PlanCost& cost,
    const ExprVector& groupingKeys,
  ...
  // Build inner GROUP BY keys: groupingKeys union distinctArgs.
  PlanObjectSet innerKeySet = PlanObjectSet::fromObjects(groupingKeys);
  innerKeySet.unionObjects(distinctArgs);
  ExprVector innerKeys = innerKeySet.toObjects<Expr>();
