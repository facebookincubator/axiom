...
Selectivity comparisonSelectivity(
  ...
  VELOX_CHECK_EQ(
      call->args().size(), 2, "Comparison operators must have exactly 2 arguments");
...
Selectivity exprSelectivity(
  ...
  if (expr->is(PlanType::kCallExpr)) {
    ...
    if (funcName == SpecialFormCallNames::kOr) {
      ...
      for (auto* arg : args) {
        disjuncts.push_back(
            exprSelectivity(state, arg, false, newConstraints));
      }
    ...
    if (isName(funcName, "isnull")) {
      VELOX_CHECK_EQ(call->args().size(), 1, "isnull must have exactly 1 argument");
...
Selectivity comparisonSelectivityImpl(
  ...
  double range_a = std::max(1.0, static_cast<double>(ah) - static_cast<double>(al));
  double range_b = std::max(1.0, static_cast<double>(bh) - static_cast<double>(bl));

  // Calculate overlap
  double overlap_low = std::max(static_cast<double>(al), static_cast<double>(bl));
  double overlap_high = std::min(static_cast<double>(ah), static_cast<double>(bh));
  ...
  if (isName(funcName, "eq")) {
    ...
    if (ah < bl) {
      ...
      double below_b = std::max(0.0, std::min(static_cast<double>(ah), static_cast<double>(bl)) - static_cast<double>(al));
    ...
    if (al > bh) {
      ...
      // Ranges overlap
      double below_a = std::max(0.0, std::min(static_cast<double>(bh), static_cast<double>(al)) - static_cast<double>(bl));
...
Selectivity columnComparisonSelectivity(
  ...
  bool canUseRange = (leftValue.min && leftValue.max &&
