...
void ParquetTpchTest::makeBucketedTables(std::string_view path) {
  ...
  for (const auto& tableInfo : tables) {
    ...
    // Parse SQL
    ::axiom::sql::presto::PrestoParser parser(connectorId, std::nullopt, pool.get());
    ...
    for (const auto& [key, value] : ctasStatement->properties()) {
      tableOptions[key] = optimizer::ConstantExprEvaluator::evaluateConstantExpr(*value);
    }
    ...
    // Create session
    auto axiomSession = std::make_shared<axiom::Session>(
        fmt::format("create_{}", targetTable));
