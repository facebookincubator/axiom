...
TEST_P(BucketedExecutionTest, rightJoinAggregation) {
...
TEST_P(BucketedExecutionTest, fullJoinAggregation) {
  addBucketedTable("t", {"a"}, 128, ROW("a", BIGINT()), 1'000);
  addBucketedTable("u", {"b"}, 128, ROW("b", BIGINT()));

  auto plan = planDistributed(parseSelect(
      "SELECT coalesce(b, a) AS key, count(*) "
      "FROM u FULL JOIN t ON b = a "
      "GROUP BY coalesce(b, a)",
      kTestConnectorId));
  AXIOM_ASSERT_DISTRIBUTED_PLAN_V2(
      plan.plan,
      matchScan("u")
          .hashJoinFull(
              matchScan("t"),
              {.keys = {{"b = a"}}, .outputColumnNames = {{"b", "a"}}})
