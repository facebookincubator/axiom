    ...
    connector_->addTable(
        ...
        ROW(
            {{"c_custkey", BIGINT()},
             ...
             {"c_comment", VARCHAR()}}),
        1000);
    // enable filter pushdown by default
    connector_->setPushdownFilters(true);
...
TEST_F(OptimizerTest, selectNoPushdown) {
  auto orderType = ROW(
      {{"o_orderkey", BIGINT()},
       {"o_custkey", BIGINT()},
       {"o_totalprice", DOUBLE()}});
  auto plan = exec::test::PlanBuilder()
                  .tableScan("orders", orderType)
                  .filter("o_totalprice < 500.0")
                  .project({"o_orderkey", "o_custkey", "o_totalprice"})
                  .planNode();

  // force the connector to reject all filters
  connector_->setPushdownFilters(false);
  ...
  //        (t2.o_orderkey:BIGINT, "t2.o_orderkey"),
  //        (t2.o_custkey:BIGINT, "t2.o_custkey"),
  //        (t2.o_totalprice:DOUBLE, "t2.o_totalprice")]
  //   Filter[1][expression: and(lt("t2.o_totalprice",500))] ->
  //         "t2.o_totalprice":DOUBLE,
  //         "t2.o_custkey":BIGINT,
  //         "t2.o_orderkey":BIGINT
  //     TableScan[0][orders]
  ASSERT_EQ(node->name(), "Project");
  auto projectNode = std::dynamic_pointer_cast<const core::ProjectNode>(node);
  ASSERT_TRUE(projectNode != nullptr);

  auto filterNode = projectNode->sources()[0];
  ASSERT_EQ(filterNode->name(), "Filter");
  auto scanFilterNode =
      std::dynamic_pointer_cast<const core::FilterNode>(filterNode);
  ASSERT_TRUE(scanFilterNode != nullptr);
  const auto& filter = scanFilterNode->filter();
  ASSERT_EQ(filter->toString(), "and(lt(\"t2.o_totalprice\",500))");

  auto scanNode = filterNode->sources()[0];
  ASSERT_EQ(scanNode->name(), "TableScan");
  auto tableScanNode =
      std::dynamic_pointer_cast<const core::TableScanNode>(scanNode);
  ASSERT_TRUE(tableScanNode != nullptr);
