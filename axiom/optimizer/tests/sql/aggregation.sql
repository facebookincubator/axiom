...
----
-- error: Distinct aggregation plan not eligible for transformation to GroupBy or MarkDistinct.
SELECT a, count(DISTINCT c) FILTER (WHERE c > 0) FROM t GROUP BY a
----
-- TODO: Velox DistinctAggregations needs to be fixed to allow literal arguments.
