...
TEST_F(UnnestTest, unnestGroupByQueries) {
  auto query =
      "SELECT  c, COUNT(b)  FROM unnest(array[1, 2, 3], array[4, 5]) WITH ORDINALITY AS t(a, b, c) GROUP BY c";
...
TEST_F(UnnestTest, unnestPrune) {
  auto query =
      "SELECT a FROM unnest(array[1, 2, 3], array[4, 5]) WITH ORDINALITY as t(a, b, c) LIMIT 1";
...
TEST_F(UnnestTest, unnestLimitPrune) {
  auto query =
      "SELECT a FROM unnest(array[1, 2, 3], array[4, 5]) WITH ORDINALITY as t(a, b, c) LIMIT 1";
...
TEST_F(UnnestTest, unnestOrderByQueriesPrune) {
  auto query =
...
TEST_F(UnnestTest, unnestGroupByQueriesPrune) {
  auto query =
      "SELECT  a, COUNT(b)  FROM unnest(array[1, 2, 3], array[4, 5]) WITH ORDINALITY AS t(a, b, c) GROUP BY a, c";
