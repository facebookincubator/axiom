...
// Verifies that an empty input table with ROLLUP still produces the expected
// default row for the global grouping set {}.
TEST_F(AggregationPlanTest, groupingSetsEmptyInput) {
  testConnector_->addTable("t", ROW({"a", "c"}, {BIGINT(), DOUBLE()}));
  SCOPE_EXIT {
    testConnector_->dropTableIfExists("t");
  };
  ...
  auto logicalPlan = lp::PlanBuilder()
                         .tableScan(kTestConnectorId, "t")
                         .rollup({"a"}, {"sum(c) as total"}, "$grouping_set_id")
                         .build();

  // ROLLUP(a) = grouping sets {a}, {}.
  // Empty input should still produce one row for the global set {}.
  auto referencePlan =
      exec::test::PlanBuilder()
          .values({makeRowVector(
              {"a", "c"},
              {makeFlatVector<int64_t>({}), makeFlatVector<double>({})})})
          .groupId({"a"}, {{"a"}, {}}, {"c"})
          .singleAggregation({"a", "group_id"}, {"sum(c) as total"})
          .project({"a", "total", "group_id"})
          .planNode();
