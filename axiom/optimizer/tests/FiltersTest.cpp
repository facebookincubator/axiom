...
TEST_F(FiltersTest, combinations) {
  ...
  for (const auto& [condition, expectedSelectivity] : conditionsWithExpected) {
    ...
    results.push_back(
        {condition, expectedSelectivity, (float)selectivity.trueFraction, passed});
  }
...
TEST_F(FiltersTest, combineConjunctsWithNulls) {
  ...
  // P(NULL) = (0.5 + 0.2) * (0.6 + 0.1) - 0.30 = 0.7 * 0.7 - 0.30 = 0.49 - 0.30 = 0.19
...
TEST_F(FiltersTest, combineDisjunctsMultiple) {
  ...
  // P(TRUE) = 1 - (1-0.2) * (1-0.3) * (1-0.1) = 1 - 0.8 * 0.7 * 0.9 = 1 - 0.504 = 0.496
...
TEST_F(FiltersTest, columnComparisonNoOverlapBelow) {
  ...
  Selectivity result = columnComparisonSelectivity(leftValue, rightValue, toName("lt"));
  EXPECT_NEAR(result.trueFraction, 1.0, 0.001);
  EXPECT_NEAR(result.nullFraction, 0.0, 0.001);
...
TEST_F(FiltersTest, columnComparisonNoOverlapAbove) {
  ...
  Selectivity result = columnComparisonSelectivity(leftValue, rightValue, toName("gt"));
  EXPECT_NEAR(result.trueFraction, 1.0, 0.001);
  EXPECT_NEAR(result.nullFraction, 0.0, 0.001);
...
TEST_F(FiltersTest, columnComparisonPartialOverlap) {
  ...
  Selectivity result = columnComparisonSelectivity(leftValue, rightValue, toName("eq"));
  ...
  // below_a should be 0 since min(bh=2500, al=1000) = 1000, 1000-bl=1500 = -500, max(0,-500)=0
  // overlap_integral = 500 * (2*2000 - 2000 - 1500) / (2 * 1000)
  //                  = 500 * 500 / 2000 = 125
...
TEST_F(FiltersTest, columnComparisonLteGte) {
  ...
  Selectivity resultLt = columnComparisonSelectivity(leftValue, rightValue, toName("lt"));
  ...
  Selectivity resultGt = columnComparisonSelectivity(leftValue, rightValue, toName("gt"));
