...
TEST_F(SqlQueryRunnerTest, executionCpuTimingUsesVeloxTaskStats) {
  ...
  options.numWorkers = 1;
  options.numDrivers = 1;
  options.componentWriterProvider = [&](std::string_view componentId)
  ...
  const auto executeCpuMetric = stats.find(
      facebook::axiom::ComponentMetrics::kCliComponent,
  ...
  EXPECT_FALSE(stats
                   .find(
                       facebook::axiom::ComponentMetrics::kRunnerComponent,
                       SqlQueryRunner::kExecuteCpuNanos)
                   .has_value());
