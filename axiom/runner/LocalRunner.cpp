...
// Collects all splits from the source, then distributes them round-robin
// across tasks. Splits are fully materialized before distribution to avoid
// interleaving generation and consumption.
  ...
  try {
    ...
    for (auto& splitAndGroup : splits) {
      tasks[taskIdx]->addSplit(
          scanId, velox::exec::Split(std::move(splitAndGroup.split)));
...
void LocalRunner::makeStages(
  ...
  auto* executor = params_.queryCtx->executor();
