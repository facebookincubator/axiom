  ...
  try {
    for (auto fragmentIndex = 0; fragmentIndex < fragments_.size();
      ...
      for (const auto& scan : scans) {
        ...
        // For bucketed tables with a bucket map, route splits to specific workers.
        ...
        do {
          ...
          if (hasBucketMap) {
            ...
            auto* hiveSplit = dynamic_cast<velox::connector::hive::HiveConnectorSplit*>(
                connectorSplit);
...
void LocalRunner::setupBuckets() {
  ...
  for (size_t fragmentIndex = 0; fragmentIndex < fragments_.size(); ++fragmentIndex) {
    const auto& fragment = fragments_[fragmentIndex];
    ...
    for (const auto& scan : scans) {
      ...
      try {
        metadata = connector::ConnectorMetadata::metadata(handle->connectorId());
      } catch (const std::exception&) {
    ...
    // Validate that all partitioned scans have the same number of partition columns
    // and matching types
    const auto& firstHandle = partitionedScans[0]->tableHandle();

    connector::ConnectorMetadata* firstMetadata = nullptr;
    try {
      firstMetadata = connector::ConnectorMetadata::metadata(firstHandle->connectorId());
    } catch (const std::exception&) {
      // Metadata not registered, skip bucket setup
      return;
    }
    ...
    for (size_t i = 1; i < partitionedScans.size(); ++i) {
      ...
      for (size_t colIdx = 0; colIdx < numPartitionColumns; ++colIdx) {
        if (!partitionColumns[colIdx]->type()->equivalent(*firstPartitionTypes[colIdx])) {
          VELOX_FAIL(
    ...
    for (const auto& scan : partitionedScans) {
      ...
      const auto* hivePartitionType =
          dynamic_cast<const connector::hive::HivePartitionType*>(partitionType);
