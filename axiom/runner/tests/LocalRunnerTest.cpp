...
TEST_F(LocalRunnerTest, splitEnumerationStatsAreRunnerScoped) {
  ...
  auto partitions =
      stats.find(std::string(RunnerMetrics::kListPartitionsCount));
  ...
  auto splits = stats.find(std::string(RunnerMetrics::kGetSplitsCount));
  ...
  EXPECT_FALSE(connectorStats.contains(
      std::string(RunnerMetrics::kListPartitionsCount)));
  EXPECT_FALSE(
      connectorStats.contains(std::string(RunnerMetrics::kGetSplitsCount)));
}
