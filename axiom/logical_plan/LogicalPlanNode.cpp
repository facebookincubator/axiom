...
folly::dynamic ValuesNode::serialize() const {
  ...
  if (std::holds_alternative<Variants>(data_)) {
    ...
    VELOX_NYI(
        "Serialization not implemented for ValuesNode with RowVectorPtr data. "
        "Requires vector serialization which is not yet implemented.");
  } else {
    obj["dataType"] = "Exprs";
...
LogicalPlanNodePtr ValuesNode::create(
  ...
  if (dataType == "Variants") {
    ...
    VELOX_NYI(
        "Deserialization not implemented for ValuesNode with RowVectorPtr data.");
  }
}
