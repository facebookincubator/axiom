...
void expressionParse(const std::string& input) {
  ParserHelper helper;
  auto parser = helper.createParser(input);
  parser->expression();
  EXPECT_EQ(0, parser->getNumberOfSyntaxErrors())
      << "Could not parse '" << input << "' " << "[" << helper.lexer_->getText()
      << "]";
}

void queryParse(const std::string& input) {
  ParserHelper helper;
  auto parser = helper.createParser(input);
  parser->query();
  EXPECT_EQ(0, parser->getNumberOfSyntaxErrors())
