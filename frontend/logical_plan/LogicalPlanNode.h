...
/// Writes data to a new or existing table using the specified column names
/// along with the schema provided by the input node.
class TableWriteNode : public LogicalPlanNode {
 public:
  /// @param outputType Metadata information generated by the write.
  /// @param connectorId The connector to be used to perform the write.
  /// @param tableName Table name.
  /// @param columnNames Column names to use for the output table.
  TableWriteNode(
      const std::string& id,
      const RowTypePtr& outputType,
      const std::string& connectorId,
      ...
      : LogicalPlanNode(NodeKind::kTableWrite, id, {input}, outputType),
        ...
        columnNames_(columnNames) {
    VELOX_USER_CHECK(
        input->outputType()->size() == columnNames_.size(),
        "Must have a number of columns equal to input schema size");
  }
